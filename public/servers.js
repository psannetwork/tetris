...
function playerRotate(dir) {
    ...
    // Tスピン判定
    if (player.matrix[1][1] === 1 && player.lastMove === "rotate") {
        const corners = [
            arena[player.pos.y - 1] && arena[player.pos.y - 1][player.pos.x - 1],
            arena[player.pos.y - 1] && arena[player.pos.y - 1][player.pos.x + 1],
            arena[player.pos.y + 1] && arena[player.pos.y + 1][player.pos.x - 1],
            arena[player.pos.y + 1] && arena[player.pos.y + 1][player.pos.x + 1],
        ];
        ...
        if (filledCorners >= 3) {
            player.score += 400; // T-Spinのスコア
                      console.log("Tスピン"); // Tスピン成功時の表示

        }
    }

    player.lastMove = "rotate"; // 最後の移動が回転だったことを記録
}
...
function playerDrop() {
    ...
    if (collide(arena, player)) {
        ...
        dropInterval = 50; // 地面に接触する直前の速度
        draw(); // 描画を更新して速度が反映されるようにする
        setTimeout(() => {
            dropInterval = originalDropInterval; // 元の速度に戻す
        }, 50); // 50ms後に元の速度に戻す
...
setInterval(setKillerTrue, 2400);
