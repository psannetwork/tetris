...
lockPiece() {
  ...
  // ★ 修正: 攻撃ゲージが 1 以上 & 赤点滅 & REN中でない場合のみガベージ追加
  let gaugeAge = performance.now() - this.gaugeTimestamp;
  if (this.attackGauge > 0 && this.gaugeTimestamp > 0 && gaugeAge >= GAUGE_TIME_RED && !this.renActive) {
    console.log("Adding garbage: gauge has charge, red blinking expired, and REN inactive.");
  ...
  this.board.clearLines((linesCleared) => {
    this.afterClearLines(linesCleared);
  });
}
