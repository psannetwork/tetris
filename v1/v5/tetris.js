...
// Dummy sending function: it logs the number of garbage lines to send.
function sendGarbage(count) {
  console.log("Sending garbage to opponent: " + count);
}
...
function processGarbageBar(firepower) {
  ...
  if (accumulated > 0) {
    ...
    if (remainder > 0) {
      sendGarbage(remainder);
    }
  } else {
    sendGarbage(firepower);
  }
}
...
function lockPiece() {
  ...
  if (lines.length) {
    ...
    setTimeout(() => {
      ...
      switch (lines.length) {
        case 1:
          pts = CONFIG.scoring.single;
          break;
        case 2:
          pts = CONFIG.scoring.double;
          break;
        case 3:
          pts = CONFIG.scoring.triple;
          break;
        case 4:
          pts = CONFIG.scoring.tetris;
          break;
      }
      score += pts;
      linesCleared += lines.length;
      ...
      currentPiece.combo = (lockedPiece.combo || 0) + 1;
      isClearing = false;
    }, CONFIG.effects.lineClearDuration);
  } else {
    currentPiece = nextPieces.shift();
    ...
    holdUsed = false;
    processFlashingGarbage();
    if (!isValidPosition(currentPiece, 0, 0) && currentPiece.y >= 0) triggerGameOver();
...
function draw() {
  ...
  for (const seg of attackBarSegments) {
    let segHeight = boardHeight * (seg.value / MAX_ATTACK);
